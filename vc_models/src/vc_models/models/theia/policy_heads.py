...
class MyModel(nn.Module):
    def __init__(self, embedding_dim, proprio_dim, history_window, neck, head):
        super().__init__()
        self.embedding_dim = embedding_dim
        self.proprio_dim = proprio_dim
        self.history_window = history_window
        self.content = nn.ModuleDict({"neck": neck, "head": head})
    ...
    def forward(self, observation: torch.Tensor) -> torch.Tensor:
        ...
        if len(self.embedding_dim) > 0:
            ...
            if self.proprio_dim > 0:
                ...
                emb_obs = self.content["neck"](emb_obs)
                ...
                self.obs_var = self.content["neck"](emb_obs)
        else:
            raise ValueError(f"input observation {observation.size()} is not from a valid spatial embedding.")
        mean = self.content["head"](self.obs_var.to(observation.device))
        return mean
...
class ConvBatchNormMLP(BatchNormMLP):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.model = MyModel(embedding_dim=self.embedding_dim, proprio_dim=self.proprio_dim,
                             history_window=self.history_window, neck=neck, head=head)
        # nn.ModuleDict({"neck": neck, "head": head})
    ...
    def get_action(self, observation: NDArray) -> Tuple[NDArray, Dict[str, Any]]:
        ...
        """
        return self.get_action_deterministic(observation)
        with torch.no_grad():
